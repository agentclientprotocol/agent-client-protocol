...
mod test_serialization {
    ...
    //
    // Mirrors the v1 coverage: locks in the stable wire contract for the
    // logout method family (method name, empty-object payload shape, and
    // the `AgentCapabilities.auth.logout` capability). Previously these
    // types were gated behind `feature = "unstable_logout"` and shipped
    // with no dedicated serialization tests.
