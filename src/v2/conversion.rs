...
mod tests {
    ...
    // -------------------------------------------------------------------
    // Logout method (stabilized in #1273).
    //
    // These conversions previously lived behind `feature = "unstable_logout"`
    // and never had cross-version tests. Now that logout is stable, we
    // exercise both directions of the conversion for every logout type,
    // including the `_meta` payload — a common source of drift when a
    // conversion is generated by hand.

    fn sample_meta(key: &str, value: &str) -> crate::v1::Meta {
        let mut meta = crate::v1::Meta::new();
    ...
    fn round_trips_logout_request() {
        let bare = v1::LogoutRequest::new();
        assert_v1_round_trip::<v1::LogoutRequest, v2::LogoutRequest>(bare.clone());
        assert_json_eq_after_v1_to_v2::<v1::LogoutRequest, v2::LogoutRequest>(bare);

        let with_meta = v1::LogoutRequest::new().meta(sample_meta("trace_id", "abc-123"));
        assert_v1_round_trip::<v1::LogoutRequest, v2::LogoutRequest>(with_meta.clone());
        assert_json_eq_after_v1_to_v2::<v1::LogoutRequest, v2::LogoutRequest>(with_meta);
    }

    #[test]
    fn round_trips_logout_response() {
        let bare = v1::LogoutResponse::new();
        assert_v1_round_trip::<v1::LogoutResponse, v2::LogoutResponse>(bare.clone());
        assert_json_eq_after_v1_to_v2::<v1::LogoutResponse, v2::LogoutResponse>(bare);

        let with_meta =
            v1::LogoutResponse::new().meta(sample_meta("logged_out_at", "2026-01-01T00:00:00Z"));
        assert_v1_round_trip::<v1::LogoutResponse, v2::LogoutResponse>(with_meta.clone());
        assert_json_eq_after_v1_to_v2::<v1::LogoutResponse, v2::LogoutResponse>(with_meta);
    }

    #[test]
    fn round_trips_logout_capabilities() {
        let bare = v1::LogoutCapabilities::new();
        assert_v1_round_trip::<v1::LogoutCapabilities, v2::LogoutCapabilities>(bare.clone());
        assert_json_eq_after_v1_to_v2::<v1::LogoutCapabilities, v2::LogoutCapabilities>(bare);

        let with_meta = v1::LogoutCapabilities::new().meta(sample_meta("ext", "example"));
        assert_v1_round_trip::<v1::LogoutCapabilities, v2::LogoutCapabilities>(with_meta.clone());
        assert_json_eq_after_v1_to_v2::<v1::LogoutCapabilities, v2::LogoutCapabilities>(with_meta);
    }

    #[test]
    fn round_trips_agent_auth_capabilities() {
        // No logout support advertised.
        ...
        // Logout advertised via `logout: {}` — the presence of the value
        // is meaningful, so make sure it isn't dropped by the conversion.
        ...
        // Logout carried alongside a top-level `_meta`.
        let full = v1::AgentAuthCapabilities::new()
            .logout(v1::LogoutCapabilities::new().meta(sample_meta("logout_ext", "true")))
            .meta(sample_meta("auth_ext", "true"));
        assert_v1_round_trip::<v1::AgentAuthCapabilities, v2::AgentAuthCapabilities>(full.clone());
        assert_json_eq_after_v1_to_v2::<v1::AgentAuthCapabilities, v2::AgentAuthCapabilities>(full);
    }

    #[test]
    fn round_trips_agent_capabilities_preserves_auth_logout() {
        // Guards against the `AgentCapabilities.auth` field being dropped
        // when converting between v1 and v2. This whole field graduated
        // from `unstable_logout` in #1273 and is the sole way logout is
    ...
    fn round_trips_client_request_logout_variant() {
        // The `ClientRequest::LogoutRequest` enum variant was also
        // previously feature-gated. If either side of the conversion
        // dropped the variant, the helper below would panic with the
        // "conversion failed" message. `ClientRequest` deliberately
        // does not implement `PartialEq`, so we exercise the conversion
        // via a JSON-equality assertion rather than value equality.
        let request = v1::ClientRequest::LogoutRequest(
            v1::LogoutRequest::new().meta(sample_meta("trace_id", "req-1")),
        );
        assert_json_eq_after_v1_to_v2::<v1::ClientRequest, v2::ClientRequest>(request);
    }
}
