...
mod tests {
    use super::*;
    use crate::v1::{ContentBlock, ErrorCode, TextContent};
    use serde_json::json;
    ...
    fn tool_call_default_kind_and_status_are_omitted() {
        ...
        // `Other` and `Pending` are the defaults and must be skipped to keep
    ...
    fn tool_kind_unknown_variants_decode_to_other() {
        // ToolKind uses `#[serde(other)]` so any new variant introduced by a
        // future spec revision must deserialize as Other rather than failing.
    ...
    fn tool_call_update_only_overwrites_present_fields() {
        ...
        // Only status changes; other fields are untouched.
        assert_eq!(call.status, ToolCallStatus::Completed);
    ...
    fn tool_call_try_from_update_requires_title() {
        ...
        assert_eq!(
            ...
            "title-required error should carry a stable, actionable message",
        );
    }
    ...
    fn tool_call_to_update_preserves_every_field() {
        // `From<ToolCall> for ToolCallUpdate` is used when re-broadcasting a
        // tool call as an update. Dropping any field here would silently
        // erase state on the receiving side.
        ...
        // And the round trip rebuilds an equivalent ToolCall.
    ...
    fn tool_call_update_flattens_fields_on_wire() {
        // `#[serde(flatten)]` means fields live at the top level of the JSON
        // object next to `toolCallId`, not nested under a `fields` key.
        let update = ToolCallUpdate::new(
            "tc_1",
            ToolCallUpdateFields::new()
                .status(ToolCallStatus::Completed)
    ...
    fn tool_call_skips_malformed_content_and_locations() {
        // Mirror of the `Plan::entries` resilience: a single bad item in
        // `content` or `locations` must not poison the whole tool call.
        let input = json!({
            "toolCallId": "tc_1",
            "title": "hi",
            "content": [
    ...
    fn tool_call_treats_outer_shape_errors_as_empty_collections() {
        // `DefaultOnError` swallows outer-shape failures for content/locations.
        let input = json!({
            "toolCallId": "tc_1",
            "title": "hi",
            "content": "oops",
    ...
    fn tool_call_update_fields_tolerate_unknown_kind_and_status() {
        // Each scalar field on the update is wrapped in `DefaultOnError`,
        // so a future enum variant arriving over the wire collapses to
        // `None` (or `Some(Other)` where the type has an `other` catch-all)
        ...
        .unwrap();
        // serde(other) on ToolKind means unknown kinds decode to Other (Some).
        assert_eq!(fields.kind, Some(ToolKind::Other));
        // ToolCallStatus has no `other` variant, so DefaultOnError -> None.
