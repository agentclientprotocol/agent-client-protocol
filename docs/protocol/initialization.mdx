...
Agents that support multi-root session workspaces advertise `sessionCapabilities.additionalDirectories`. Clients **MUST** only send `additionalDirectories` on session lifecycle requests or `session/list` when that capability is present.
