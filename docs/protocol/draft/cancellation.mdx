...
In order to provide a consistent approach to cancellation, ACP defines a `$/cancel_request` notification that can be sent to cancel requests if the recipient declared support.
...
- The other party **MAY** send `$/cancel_request` to cancel requests if the recipient declared support.
- If an implementation does not declare support, `$/cancel_request` notifications **SHOULD** be ignored.
- Either party **MAY** send `$/cancel_request` regardless of if they declare support themselves, as long as the recipient supports cancellation.
