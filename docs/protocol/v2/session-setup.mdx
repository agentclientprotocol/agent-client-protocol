...
The Agent **MUST** cancel any ongoing work for that session as if [`session/cancel`](/protocol/v2/prompt-turn#cancellation) had been called, then free the resources associated with the session.
