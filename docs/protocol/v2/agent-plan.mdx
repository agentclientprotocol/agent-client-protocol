...
Agents may share plans with Clients through [`session/update`](/protocol/v2/prompt-turn#3-agent-reports-output) notifications, providing real-time visibility into their thinking and progress.

`plan_update` carries a `plan` object with a `type` discriminator and a required `id` so Clients can track multiple plans independently.
