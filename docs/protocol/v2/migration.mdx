...
ACP v2 is a consolidation release. It redesigns the prompt lifecycle around session state, unifies streaming and non-streaming updates behind upsert semantics, makes the schema forward-compatible by default, and removes protocol surface that the ecosystem had already moved away from.

This guide covers everything an implementer familiar with v1 needs to move an Agent or Client to v2. All examples are JSON wire messages and apply to any language.
...
| `logout` (gated by `agentCapabilities.auth.logout`)                                                 | Renamed to `auth/logout`; always required, no capability marker                                        |
| `session/new`                                                                                       | Unchanged name; `mcpServers` now optional; response no longer includes `modes`                         |
| `session/load`                                                                                      | **Removed** — use `session/resume` with `"replayFrom": { "type": "start" }`                            |
| `session/resume`                                                                                    | Unchanged name; gains optional `replayFrom`; baseline (no capability marker)                           |
| `session/list`, `session/close`                                                                     | Baseline when `capabilities.session` is present (no capability markers)                                |
...
v1 used role-specific field names: the Client sent `clientCapabilities` and optional `clientInfo`, and the Agent returned `agentCapabilities` and optional `agentInfo`. v2 uses the same two field names in both directions — `capabilities` and `info` — and `info` is now **required** on both sides. This keeps symmetric connections (such as agent-to-agent) from needing role-specific names.
...
In v1, capability support was a mix of booleans (`"image": true`) and objects (`"list": {}`). In v2, every support marker is an object: supplying `{}` (or an object with fields) means supported, and omitting the key or supplying `null` means unsupported. Booleans remain appropriate only for actual data or configuration _inside_ an already-advertised capability.

This means checks like `promptCapabilities.image === true` become presence checks: `capabilities.session.prompt.image != null`. The object shape leaves room for each capability to grow fields without another breaking change.
...
- `capabilities.session` itself is **optional**, so agents that don't do session-based prompting (for example, agents that only serve specialized extension surfaces) can omit it entirely.
...
- The v1 Client capabilities `fs` and `terminal` are removed entirely (see [Client file system and terminals](#client-file-system-and-terminals-removed)). Stable v2 currently defines no standard Client capability fields; the object remains as an extension point.
...
- **Omitted** `content` — leave existing content unchanged (useful for updating `_meta` or other fields without resending content).
- `content: null` **or** `content: []` — clear the message content.
- **Concrete array** — replace all content currently stored for that message, including content accumulated from earlier chunks.
...
New entities introduced in v2 follow the same rule (`messageId` on messages, `planId` on plans). If you generate types from the schema, expect these renames to surface as compile-time breaks — a good checklist by itself.
...
- **Open enums and tagged unions.** Every enum-like string (`sessionUpdate`, tool `kind`, tool `status`, stop reasons, plan types and entry priorities/statuses, content types, content roles, diff operations and file types, permission subjects, permission option kinds and outcomes, auth method types, config option types and categories, command input types, MCP transports, …) accepts unknown values. Values beginning with `_` are reserved for implementation-specific extensions (e.g. `_mycompany_widget`); unknown values _without_ a leading `_` are reserved for future ACP versions. Receivers **SHOULD** preserve unknown values when storing, replaying, proxying, or forwarding, and either render them generically or fall back safely. Do not mint non-underscore custom values.
...
- **Explicit nullability.** Throughout the v2 schema, optional fields document whether `null` is equivalent to omission; for upsert payloads they are deliberately different (omitted = unchanged, `null` = clear).
...
12. **Client surface.** Remove all `fs/*` and `terminal/*` calls and terminal tool content; rely on client-provided MCP servers for client-side access.
13. **MCP config.** Require and emit `type` discriminators; drop SSE; advertise `session.mcp.stdio` / `session.mcp.http` as appropriate; accept omitted `mcpServers`.
...
- v1 has no way to express message-content replacement or clearing, metadata-only message updates, appended tool-call content chunks, structured diff operations (moves, copies, deletes, binary changes), or permission prompt copy separate from tool-call state.
