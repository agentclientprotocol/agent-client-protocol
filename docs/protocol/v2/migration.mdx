...
`output` is a replacement snapshot for replay, correction, or resynchronization. Its `data` and `truncated` fields are required. `data` is RFC 4648 base64-encoded bytes. `truncated: false` means the snapshot contains the complete output from the start of the terminal. `truncated: true` means one or more bytes from the beginning were omitted and the snapshot contains only the retained suffix; it does not mean the end was cut off or that more output will follow. The flag describes completeness and does not change replacement behavior: Clients must not splice an older locally stored prefix onto either snapshot. Clients should indicate an omitted prefix. Rich terminal reconstruction from a truncated suffix is best-effort because the missing bytes may have established parser or screen state.

A concrete `exitStatus` marks the terminal as exited. Its optional nullable `exitCode` and `signal` fields use omission or `null` when unavailable. Conventional POSIX signal names include `SIGTERM`, `SIGKILL`, and `SIGINT`; other platforms may use platform-specific names. For example: `"exitStatus": { "signal": "SIGTERM" }`.
...
8. **Diffs.** Replace `oldText`/`newText` with `changes` (+ `fileType`/`mimeType` where known) and provide a `git_patch` whenever feasible.
...
7. **Diffs.** Render `patch.diff` (git patch format) when present. Drive file trees and summaries from `changes`. Handle patch-less diffs (binary, symlink, directory).
...
5. **Fixtures.** Cover the full prompt lifecycle, resume replay, cancellation, permission flow, message replacement and clearing, tool-call content chunks, terminal snapshot replacement, split and invalid UTF-8 bytes, truncation, exit state, command subjects, structured diffs, and JSON-RPC batches.
