...
- User prompts sent via [`session/prompt`](/protocol/v2/prompt-turn#1-user-message)
- Language model output reported through [`session/update`](/protocol/v2/prompt-turn#3-agent-reports-output) notifications as message updates or streamed chunks
- Progress updates and results from [tool calls](/protocol/v2/tool-calls), including streamed tool-call content chunks
...
capability](/protocol/v2/initialization#prompt-capabilities) when included in
prompts.
