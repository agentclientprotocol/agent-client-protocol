...
A prompt turn represents a period of work in a session, often starting with a user message and continuing until the [Agent](/protocol/v2/overview#agent) reports that the session is idle again. This may involve multiple exchanges with the language model and tool invocations.
