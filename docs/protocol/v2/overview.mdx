...
<Step title="Prompt Turn">
   ...
   - Turn ends when the Agent sends an idle `state_update` session update with a stop reason
</Step>
</Steps>
...
<ResponseField
  name="authenticate"
  post={[<a href="/protocol/v2/schema#authenticate">Schema</a>]}
>
...
>
  [Send user prompts](/protocol/v2/prompt-turn#1-user-message) to the Agent.
</ResponseField>
...
>
  [Cancel ongoing operations](/protocol/v2/prompt-turn#cancellation) (no
  response expected).
</ResponseField>
...
>
  [Send session updates](/protocol/v2/prompt-turn#3-agent-reports-output) to
...
- Review the [Prompt Turn](/protocol/v2/prompt-turn) lifecycle
