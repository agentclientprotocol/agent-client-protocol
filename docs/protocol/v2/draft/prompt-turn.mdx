...
A prompt turn represents a period of work in a session, often starting with a user message and continuing until the [Agent](/protocol/v2/draft/overview#agent) reports that the session is idle again. This may involve multiple exchanges with the language model and tool invocations.
...
Upon receiving a prompt request, the Agent **MUST** respond once it has accepted the prompt. The response body is empty because turn completion is reported through `state_update` notifications, not through the `session/prompt` response:
...
If there is no pending work, the Agent **MUST** report that the session is idle with a `state_update` notification. When the idle transition ends a turn, the Agent **MUST** include the corresponding [`StopReason`](#stop-reasons):
...
Agents **MAY** stop the turn at any point by sending an idle `state_update` session update with the corresponding [`StopReason`](#stop-reasons).
...
The cycle returns to [step 3](#3-agent-reports-output), continuing until the language model completes its response without requesting additional tool calls or the turn gets stopped by the Agent or cancelled by the Client.

## Stop Reasons

When an Agent stops a turn, it must specify the corresponding `StopReason` on an idle `state_update` session update:
