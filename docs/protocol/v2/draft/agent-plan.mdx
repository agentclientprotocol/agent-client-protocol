...
`plan_update` carries a `plan` object with a `type` discriminator, and every plan format includes a required `id` so Clients can track multiple plans independently.
...
The draft unstable schema includes markdown plans, file-backed plans, and explicit plan removal. These operations are not part of the stable schema yet.
