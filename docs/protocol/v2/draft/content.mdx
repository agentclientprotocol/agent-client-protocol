...
- User prompts sent via [`session/prompt`](/protocol/v2/draft/prompt-turn#1-user-message)
- Language model output reported through [`session/update`](/protocol/v2/draft/prompt-turn#3-agent-reports-output) notifications as message updates or streamed chunks
- Progress updates and results from [tool calls](/protocol/v2/draft/tool-calls), including streamed tool-call content chunks
...
Content block `type` values can be custom or future variants when the receiver can preserve or display them generically. Custom content block types **MUST** begin with `_`. Unknown non-underscore content block types are reserved for future ACP variants.
...
capability](/protocol/v2/draft/initialization#prompt-capabilities) when included
in prompts.
