...
<ParamField path="subject" type="RequestPermissionSubject" required>
  The operation requiring permission. For tool calls, use `type: "tool_call"`
  with a `toolCall` update containing details about the operation. Custom or
  future subject types may appear; clients that do not understand a subject
  should preserve it when proxying and use a generic permission prompt or
  decline according to policy.
</ParamField>
