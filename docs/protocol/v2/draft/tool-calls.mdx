...
Tool calls represent actions that language models request Agents to perform during [active session work](/protocol/v2/draft/prompt-lifecycle). When an LLM determines it needs to interact with external systems—like reading files, running code, or fetching data—it generates tool calls that the Agent executes on its behalf.
...
- `cwd` is the command's working directory and **MUST** be absolute when
  supplied.
- `output` is a full replacement snapshot. Its required `data` is RFC 4648
  base64-encoded bytes with JSON Schema `format: "byte"`; required `truncated`
  reports whether one or more bytes from the beginning were omitted.
  `truncated: false` means the snapshot contains the complete output from the
  start of the terminal. `truncated: true` means it contains only the retained
  suffix.
  This flag describes completeness and does not change replacement behavior;
  Clients **MUST NOT** splice an older locally stored prefix onto the snapshot.
- A concrete `exitStatus` marks the terminal as exited. Its optional nullable
  `exitCode` and `signal` report known exit information; omission and `null`
  both mean the value is unavailable. Conventional POSIX signal names include
  `SIGTERM`, `SIGKILL`, and `SIGINT`; other platforms may use platform-specific
  names. Terminal exit and tool call status are independent.
- `_meta` follows the same patch semantics.

For example, a process terminated by a POSIX termination signal can report
`"exitStatus": { "signal": "SIGTERM" }`.

Clients **SHOULD** indicate when earlier output was omitted. A truncated
snapshot is a retained byte suffix, not a serialized terminal-screen
checkpoint, so terminal-emulator reconstruction from it is best-effort. The
omitted prefix may have established parser, cursor, mode, or visible-screen
state. Faithful replay requires an untruncated snapshot from the beginning of
the terminal stream.
