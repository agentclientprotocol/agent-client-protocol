...
<ResponseField name="unknown">
  The Agent cannot currently determine foreground activity. This experimental
  state is introduced by the subagents RFD and gated by `unstable_subagents` in
  the Rust schema. For example, a remote child feed may be lost while ACP
  remains connected.
</ResponseField>
...
separately if a control becomes unavailable. See the experimental
[subagents RFD](/rfds/subagents) for v1 and recovery behavior.
