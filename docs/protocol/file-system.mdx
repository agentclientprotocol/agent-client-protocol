...
Filesystem `path` values are always absolute. If a session was created with [`additionalDirectories`](./session-setup#additional-directories), Clients that enforce workspace boundaries **MUST** authorize `fs/*` requests against the full ordered root set `[cwd, ...additionalDirectories]`, not `cwd` alone.
...
<ParamField path="path" type="string" required>
  Absolute path to the file to read. Clients that enforce session workspace
  boundaries validate this path against the session's effective root set.
</ParamField>
...
Clients that enforce session workspace boundaries validate this path against the session's effective root set.
