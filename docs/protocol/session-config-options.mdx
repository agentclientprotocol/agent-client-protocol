...
<Info>
  ...
  them instead of the [`modes`](/protocol/session-modes) field. Modes will be
  removed in a future version of the protocol.
</Info>

## Initial State

During [Session Setup](/protocol/session-setup) the Agent **MAY** return a list of configuration options and their current values:
...
Session Config Options supersede the older [Session Modes](/protocol/session-modes) API. However, during the transition period, Agents that provide mode-like configuration **SHOULD** send both:
...
<Card icon="gears" horizontal href="/protocol/session-modes">
