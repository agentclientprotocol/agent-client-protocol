...
Agents can treat a session as an ordered root set (`[cwd, ...additionalDirectories]`) and perform implementation-defined discovery across declared roots. ACP remains agnostic to repository conventions, instruction layout, and skill naming.
...
- Enable generic multi-root discovery by agents without standardizing any particular directory layout.
- Expand the filesystem scope without changing the existing RPC method structure.

#### Non-goals

- Defining any required directory names, such as `.agents`, `skills/`, or similar.
...
Agents MAY perform implementation-defined discovery across any declared root. For example, an agent MAY look for instruction files, skill directories, or repository-local configuration within one or more roots. When an agent performs such discovery across multiple roots, it SHOULD use the ordered root list `[cwd, ...additionalDirectories]` as its search order.

ACP does not define:

- required directory names;
- whether an agent MUST perform any discovery at all;
- precedence rules when multiple roots contain similar artifacts; or
...
No. ACP remains agnostic to directory naming and contents. This proposal only defines additional accessible roots.
