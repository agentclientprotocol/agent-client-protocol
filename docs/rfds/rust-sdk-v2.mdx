...
We propose to replace the current Rust SDK with [`sacp`](https://github.com/symposium-dev/symposium-acp/), developed as part of the [Symposium project](https://symposium.dev). The new SDK addresses the limitations above through a builder-based API with explicit connection semantics.
...
For more complex MCP servers, you can use the standard rmcp API via the `sacp-rmcp` crate:
...
AgentToClient::builder()
    .on_receive_notification(
        ...
        sacp::on_receive_notification!(), // <-- Hacky macro argument required
    )
```
...
ProxyToConductor::builder()
    .on_receive_notification_from(
        sacp::Agent, // <-- Receive from the agent
        async |notif: SessionNotification, cx| {
            cx.send_notification_to(sacp::Client, notif)?;
            //                      -----------
            //                   Send to the client
            Ok(())
        },
...
```rust
use sacp::link::{JrLink, LinkDirection};
...
Currently, `sacp` contains both generic JSON-RPC machinery (builder patterns, message handling, connection management) and ACP-specific types (link types, schema integration). In the future, it might be valuable to split the generic JSON-RPC layer into its own crate.
...
The design principles in SACP - builder patterns, context parameters, directional naming, component abstractions - aren't Rust-specific. They represent good SDK design that could inform TypeScript and other language SDKs.

This doesn't mean other SDKs should be ports of SACP. Each language has its own idioms. But the core ideas (explicit message ordering, composable components, context in callbacks) translate across languages.
...
SACP is organized into several crates with clear responsibilities:

- **`sacp`** - Core SDK with builder patterns, link types, and component abstractions
- **`sacp-tokio`** - Tokio runtime integration (spawn, timers, I/O)
- **`sacp-proxy`** - Extensions for implementing proxies (successor forwarding, message transformation)
- **`sacp-rmcp`** - Bridge to the rmcp crate for MCP integration
...
The transition from the current SDK to SACP can be gradual:

1. **Publish SACP crates** under the ACP organization namespace
2. **Deprecate current SDK** with clear migration guidance
3. **Provide compatibility layer** for common patterns (optional, if demand exists)
...
### What changes are needed relative to the existing `sacp` crate?

The existing `sacp` crate is aligned but we are in the process of changing how response messages work to simplify the implementation of the conductor. Before stabilzing though we should do a thorough review of the methods and look for candidates that can be removed or simplified.

The existing conductor is feature complete but the support for MCP-over-ACP needs a few minor improvements (in particular, it should detect when the agent only supports stdio bridging and not attempt to use HTTP, which it currently does not).
