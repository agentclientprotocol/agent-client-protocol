...
This approach minimizes the responsibilities of a proxy to what can be covered by a single communication channel. We anticipate proxies being created by isolated WebAssembly components and we want to minimize the capabilities they need. It also removes what would be redundant logic wherein each proxy would have to know how to start its successors (and would know what its successors *are*, which they shouldn't know).

The conductor handles all process management, capability negotiation, and message routing, allowing proxies to focus purely on their transformation logic.
