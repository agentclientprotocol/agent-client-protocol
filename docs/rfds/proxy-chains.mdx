...
We add two ACP methods for proxy communication:
- **`proxy/successor/message`**: Proxy sends a message to its successor via the conductor
- **`proxy/successor/receive`**: Conductor delivers a message from the successor back to the proxy
...
sequenceDiagram
    ...
    P1->>Conductor: proxy/successor/message (enhanced prompt)
    Conductor->>P2: enhanced prompt
    P2->>Conductor: proxy/successor/message (filtered prompt)
...
- When a proxy forwards an `initialize` via `proxy/successor/message`, the conductor determines whether the successor is another proxy or the agent, and sends the appropriate method
...
**Proxy Messages:**
```json
// Proxy sends message to successor (request or notification determined by presence of id)
{"method": "proxy/successor/message", "params": <ACP_MESSAGE>}

// Conductor delivers message from successor
{"method": "proxy/successor/receive", "params": <ACP_MESSAGE>}
```

Whether the wrapped message is a request or notification is determined by the presence of an `id` field in the ACP message, following JSON-RPC conventions.
...
sequenceDiagram
    ...
    P1->>C: proxy/successor/message (initialize)
    ...
    P2->>C: proxy/successor/message (initialize)
    ...
    C-->>P2: proxy/successor/receive (InitializeResponse)

    P2-->>C: InitializeResponse (mcp_acp_transport: true)
    C-->>P1: proxy/successor/receive (InitializeResponse)
