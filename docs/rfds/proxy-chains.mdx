...
- When a proxy forwards an `initialize` via `proxy/successor/request`, the conductor determines whether the successor is another proxy or the agent, and sends the appropriate method
...
sequenceDiagram
    ...
    P1->>C: proxy/successor/request (initialize)
    ...
    P2->>C: proxy/successor/request (initialize)
