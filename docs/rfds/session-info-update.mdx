...
2. **Align with `SessionInfo` structure**:
   ...
   - **Important**: `SessionInfoUpdate` must stay aligned with the mutable metadata fields of `SessionInfo` - immutable fields such as workspace roots do not need corresponding update fields
