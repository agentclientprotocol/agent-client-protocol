...
Introduce a standardized per-request cancellation mechanism for the Agent Client Protocol, inspired by the Language Server Protocol (LSP), to enable a more granular cancellation of requests where individual JSON-RPC requests can be cancelled one by one.
...
interface CancellationCapabilities {
  supportsRequestCancellation?: boolean; // Default: false
}
...
- A party **MUST** declare `supportsRequestCancellation: true` in their capabilities to handle `$/cancelRequest` notifications for their requests
...
- The standard JSON-RPC "method not found" mechanism will handle unsupported requests
