...
- MCP elicitation is tool-call-scoped; ACP elicitation may be request-scoped, session-scoped, or standalone
...
`sessionId` and `requestId` are both optional. In general, elicitation is not required to be tied to any specific session or request.
