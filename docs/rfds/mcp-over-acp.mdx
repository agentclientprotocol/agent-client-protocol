...
The Rust implementation and this RFD are being revised together. The response carrier is defined in the shared schema; the owned-service, bounded-transport, and HTTP re-export changes require integrated verification before this draft is ready to stabilize. Independent item-count limits are not evidence of end-to-end memory bounds, and dropping a task handle is not evidence that its work stopped.
