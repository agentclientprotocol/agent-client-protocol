...
The [`session/list` RFD](/rfds/session-list) introduced the ability for clients to enumerate sessions. However, there's no standard way to remove sessions from this list. Users accumulate sessions over time and have no way to clean them up.

Without `session/delete`:

- Users cannot remove old, abandoned, or mistakenly created sessions
- Session lists grow unbounded, making it harder to find relevant sessions
- Clients must implement non-standard deletion mechanisms or rely on agent-specific cleanup policies
- Users have no control over their session history

## What we propose to do about it

> What are you proposing to improve the situation?

Add a `session/delete` JSON-RPC method that is capability-gated. Clients opt in via `sessionCapabilities.delete`; agents advertise this capability at initialization if they support deletion.
...
- **User control**: Users can manage their session history, removing sessions they no longer need
- **Cleaner session lists**: Session browsers stay manageable as users clean up old conversations
- **Consistent behavior**: All ACP clients can offer session deletion using the same protocol method
- **Foundation for more features**: Batch delete, undo, archiving, etc. can build on this base
