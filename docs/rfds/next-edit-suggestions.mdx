...
A suggestion is one of two kinds: an **edit** (text changes) or a **jump** (navigate to a different file).
...
A response may contain a mix of edit and jump suggestions. The client decides how to present them (e.g. inline ghost text for edits, a navigation hint for jumps).
