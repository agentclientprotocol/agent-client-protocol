...
The **client** advertises its own NES-related capabilities in the `initialize` request. Currently, the client can declare which well-known IDE actions it supports by advertising them as keys under `clientCapabilities.nes.ideActions`. The agent reads these and may later include `"action"` kind suggestions that reference them.
...
Each key in `ideActions` is the ID of a well-known action (see [Well-known IDE actions](#well-known-ide-actions) below). Agents should only suggest actions that the client has advertised.
