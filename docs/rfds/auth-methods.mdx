...
Author(s): [anna239](https://github.com/anna239), [benbrandt](https://github.com/benbrandt)
...
A user can enter a key and a client will pass it to the agent as an env variable. Requires `"type": "env_var"`.
...
- `varName` (required): The environment variable name the client should set.
- `link` (optional): URL where the user can obtain their key.
