...
The `session/request_permission` method should also carry this same `ToolCallUpdate` shape for tool-call permissions. In v2, the request uses a `subject` tagged union; tool-call permissions use `type: "tool_call"` and carry the update in the subject's `toolCall` field, so the permission UI receives the same patch/upsert payload shape as session updates while leaving room for future permission subjects.
...
- 2026-07-02: Updated permission requests to carry tool-call updates through the `subject` tagged union.
