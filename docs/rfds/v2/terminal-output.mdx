...
- `output`: a replacement snapshot containing required `data` and `truncated` fields. `data` is RFC 4648 base64-encoded bytes and has JSON Schema `format: "byte"`. `truncated: false` means `data` contains the complete output from the start of the terminal through the snapshot. `truncated: true` means one or more bytes from the beginning were omitted, usually because of a retention limit, and `data` contains only the retained suffix. It does not mean bytes were omitted from the end or that more output will necessarily follow. `truncated` describes completeness; it does not change replacement behavior. In both cases, the snapshot replaces all previously stored bytes for the terminal, and Clients **MUST NOT** splice an older locally stored prefix onto it. Later chunks append to the replacement.
- `exitStatus`: a concrete object marks the terminal as exited. Its `exitCode` and `signal` fields are optional and nullable; omission and `null` are equivalent and mean that value is unavailable. Agents should use conventional platform signal names. POSIX examples include `SIGTERM`, `SIGKILL`, and `SIGINT`; other platforms may use platform-specific names. `exitStatus: null` clears a previously reported exit status.
- `_meta`: follows the same omitted, `null`, and concrete-object patch semantics.
...
Clients **SHOULD** indicate when earlier output was omitted. A truncated snapshot is a retained byte suffix, not a serialized terminal-screen checkpoint: it may begin in the middle of a UTF-8 code point or escape sequence, and omitted bytes may have established terminal modes, cursor state, or visible content. Terminal-emulator reconstruction from a truncated snapshot is therefore best-effort. Faithful replay requires an untruncated snapshot from the beginning of the terminal stream.
...
This initial design does not add byte offsets or sequence numbers. Untruncated full replacement snapshots provide the faithful replay, correction, and resynchronization operation, while truncated snapshots provide bounded best-effort recovery and chunks provide the efficient live append operation.
...
4. Mark snapshot and chunk `data` as base64 bytes with JSON Schema `format: "byte"`.
5. Add the `command` permission subject with its required command and absolute working directory plus optional associations.
6. Update v2 protocol documentation, migration guidance, generated schemas, and SDK tests.
7. Keep v1 and all Client terminal execution methods unchanged.
...
Repeatedly replacing the full output buffer is inefficient for live output, while an append-only stream is insufficient for replay, correction, truncation, and resynchronization. The two operations have distinct, unambiguous semantics.
