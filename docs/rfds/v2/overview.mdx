...
- Make `session/request_permission` carry an extensible `subject` tagged union. Tool-call permissions use `type: "tool_call"` with the same `ToolCallUpdate` payload shape as session updates, while future permission subjects can be added without changing the common request fields.
...
- 2026-07-02: Recorded that v2 permission requests use an extensible `subject` tagged union, with tool-call permissions represented as `type: "tool_call"`.
