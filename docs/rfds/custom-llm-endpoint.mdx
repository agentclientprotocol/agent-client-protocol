...
interface RestoreProvidersRequest {
   ...
   * Omitted or empty restores all providers.
   */
...
This restores every provider. To restore only a subset, include `params` with
`apiProtocols`, for example `{ "apiProtocols": ["openai"] }`.
...
3. **List semantics**: `providers/list_api` returns the supported provider APIs (protocol types) and the current effective non-secret routing config (`current`). Providers SHOULD remain discoverable in the list after `providers/restore`, and while disabled.
...
6. **Restore semantics**: `providers/restore` restores every listed `apiProtocol` to the agent's default configuration. If `params` is omitted, or `apiProtocols` is omitted or empty, it restores every provider to its default configuration. Explicit `null` is not equivalent to omission and is invalid. If the agent has no default for a protocol, the provider becomes disabled (`current` omitted or `current: null` in `providers/list_api`). Restoring a protocol that is already at its default, or that is unknown to the agent, SHOULD be treated as success (idempotent behavior).
