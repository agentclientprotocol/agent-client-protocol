...
- [Intelligent Terminal](https://github.com/microsoft/intelligent-terminal) — Windows Terminal fork with native agent integration, driving ACP-compatible agent CLIs over stdio

## Desktop and Web

- [ACP UI](https://github.com/formulahendry/acp-ui) (Windows, macOS, Linux, iOS, Android, Web)
- [AcpAgent Client](https://github.com/ClickPM/AcpAgentClient) — open-source (GPL-3.0) multi-agent desktop client built on the official Rust SDK: one-click agent install from the ACP Registry, Agent Auth and Terminal Auth, file browser, embedded terminals, and an ACP traffic inspector (Windows)
...
- [Perch](https://github.com/chenweigao/perch) — native macOS workspace for remote coding agents over SSH; drives ACP agents such as DeepSeek Harness with approval cards, stop, model and reasoning pickers, and session resume through a remote loopback broker (macOS)
- [Pilion Browser](https://github.com/echoVic/pilion-browser) — desktop browser that connects any ACP agent, local or over SSH, and hands its own tabs to the agent through MCP with approvals, human takeover, and session resume (macOS, Windows, Linux)
...
- [ClawX](https://github.com/ValueCell-ai/ClawX) — desktop GUI for OpenClaw agents, with a chat UI that talks to them over ACP
- [OmniGet](https://github.com/tonhowtf/omniget) — desktop app for local coding agents with permissions, undo, jobs, and a built-in MCP server (Windows, macOS, Linux)
- [Paca](https://github.com/Paca-AI/paca) — self-hosted, AI-native project management where humans and ACP agents share the same board and sprints
...
- [TermFold](https://github.com/Bibarud/TermFold) (Android) — runs ACP agents on the device itself: a bundled Ubuntu environment with a native chat for Claude, Codex, OpenCode, Cursor, Pi and more, with permissions, plans, model and reasoning pickers, and session resume; no PC or server needed
...
- [cc-connect](https://github.com/chenhg5/cc-connect) (Feishu/Lark, DingTalk, Slack, Telegram, Discord, LINE, WeCom) — bridges local coding agents to messaging platforms
...
- [AgentOS](https://github.com/rivet-dev/agentos) — with built-in ACP agents, and sessions created, managed, and resumed over ACP
...
- [Octop](https://github.com/TencentCloud/Octop) — self-hosted multi-user AI assistant with bidirectional ACP support for IDE and terminal agents
