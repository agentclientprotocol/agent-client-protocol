...
Maintainers are responsible for components within the ACP project. These generally are independent repositories such as language-specific SDKs. Maintainers may adopt their own rules and procedures for making decisions. Maintainers are expected to make decisions for their respective projects independently, but can defer or escalate to the core maintainers when needed.
